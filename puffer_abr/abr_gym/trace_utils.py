...
def round_step_size(a: Union[np.ndarray, float], 
        step_size: Optional[float] = MIN_TIME_CHUNK) -> Union[np.ndarray, float]:
    """
    Round the time of a series to the nearest integer multiple of step size
    Args:
        a: Arraylike, the time to round
...
def chunk_traces(traces: Iterable[Tuple[Trace, TraceInfo]]) -> pd.DataFrame:
    """
    Create a pd.DataFrame with all the traces as throughput over time.
    The pd.DataFrame will have columns [trace_idx, time, throughput].
    Args:
        traces: The traces, as returned by abr_gym.trace_loader.load_traces
    Returns:
        The throughput over time dataframe
    """
    processed_traces = []
    for i, (trace, _) in enumerate(traces):
        start_idx = np.random.choice(
            max(trace.network_data.shape[0] - MIN_TRACE_LEN, 1))
        end_idx = start_idx
        while trace.network_data[end_idx, 0] - \
                trace.network_data[start_idx, 0] < MAX_TIME and \
                end_idx < trace.network_data.shape[0]-1:
            end_idx += 1
        max_time = trace.network_data[end_idx, 0] - \
            trace.network_data[start_idx, 0]
        max_time = round_step_size(max_time)
        processed_trace = np.ones(
            (int(max_time/MIN_TIME_CHUNK), len(FEATURES_IDX)+1)) * -1
        processed_trace[:,0] = i
        processed_trace[:,1] = np.arange(0, max_time, MIN_TIME_CHUNK)
        temp_trace = np.concatenate(
            [np.expand_dims(trace.network_data[start_idx: end_idx, i], 1) 
            for i in FEATURES_IDX], axis=1)
        temp_trace[:, 0] = temp_trace[:, 0] - temp_trace[0, 0]
        indices = round_step_size(temp_trace[:, 0]) / MIN_TIME_CHUNK
        indices = np.clip(indices, 0, processed_trace.shape[0]-1)
        processed_trace[indices.astype(np.int32), 2:] = temp_trace[:, 1:]
        row = temp_trace[0, 1:]
        for i in range(processed_trace.shape[0]):
            if processed_trace[i, -1] < 0:
                processed_trace[i, 2:] = row
            else:
                row = processed_trace[i, 2:]
        processed_trace = processed_trace.tolist()
        processed_traces.extend(processed_trace)
    return pd.DataFrame(processed_traces, columns = COLUMNS)

def extract_trace_features(traces: Iterable[Tuple[Trace, TraceInfo]], 
        settings: Optional[Path] = None) -> pd.DataFrame:
    """
    Extract and return all the features of the given traces.
    Args:
        traces: The traces, as returned by abr_gym.trace_loader.load_traces
        settings: (Optional) A Path to the json file specifying tsfresh feature extractor settings
    Returns:
    ...
    extracted_features = extract_features(traces, column_id="id", 
